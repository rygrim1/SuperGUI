...
def plotbands(bands_dat, bands_out, bands_in, bg, fermi, fermi_check, fermi_shift,
    ...
    def bandplot(bands_dat_gnu, bands_out, bands_in, bg, fermi, fermi_check, fermi_shift, figure, save_key, label, band_lw, only_bands, gs1):
        ...
        q_pts=find_path(bands_out, ax)
        k_minus_q=np.array(list(set(kpt).symmetric_difference(q_pts)))
        kpts=np.sort(np.concatenate((k_minus_q, q_pts)))
        ...
        bg_shift=[0 if fermi_shift else fermi][0]
#        if fermi_shift:
#            bg_shift=0
#        else:
#            bg_shift=fermi
        ...
        if bg:
            for eig in gap_ticks:
                if (eig[0] in [min(q_pts), max(q_pts)] or eig[0] not in q_pts) and len([e for e in gap_ticks if eig[1]==e[1]]) > 1:
                    continue
                ax.plot([eig[0]], [eig[1]], marker='x', markersize=7, color='red')
        ...
        if fermi_check:
            ax.text(0, ev_b + abs(abs(ev_b) - ev_a)/30, "Fermi level: {} eV".format(fermi), fontsize=10, color='red')
            if not fermi_shift:
                ax.plot([min(kpt),max(kpt)],[fermi, fermi], color='red', linewidth=1.0)
        for j in np.unique(q_pts, axis=0):
            x1 = [j, j]
            x2 = [axis[2], axis[3]]
            ax.plot(x1, x2,'--', linewidth=1.0, color='black', alpha=0.75)
        ax.yaxis.set_major_locator(MaxNLocator(nbins=5, integer=True))
        #PbCO3=['\u0393', 'X', 'S', 'Y', '\u0393', 'Z', 'U', 'R', 'T', 'Z', 'Y', 'T', 'U', 'X', 'S', 'R']
        ax.set_xticklabels(['X', '\u0393', 'Y', 'L', '\u0393', 'Z', 'N', '\u0393', 'M', 'R', '\u0393'], fontdict=font)
        #ax.set_xticklabels(PbCO3, fontdict=font)
...
def dosplot(dos_files, figure, save_key, label, fermi, y_range, x_range, col_keys, fermi_check, fermi_shift, only_pdos, gs1, fill, dos_lw):
    ...
    ax.legend([v[0] for v in leg_labels.values()], bbox_to_anchor=(1, 0), loc=4, borderaxespad=0, fontsize=8)
    if save_key:
        figure.savefig(path+label.replace(" ",".")+'.pdf', format='pdf', dpi=1200)   

def bands_and_pdos(bands_dat, bands_out, bands_in, fermi, ev_a, ev_b, 
                   figure, save_key, label, band_lw, dos_lw, bg, fermi_check, fermi_shift, dos_files, x_range, col_keys, fill, ratio):
    ratio=[[int(i) for i in ratio.split(":")]][0]
    gs1 = gs.GridSpec(1,2, width_ratios=ratio, figure=figure)
    ...
    figure.suptitle(label, fontname='serif', fontsize=14, y=0.93)
    if save_key:
        figure.savefig(path+label.replace(" ",".")+'.pdf', format='pdf', dpi=1200)
